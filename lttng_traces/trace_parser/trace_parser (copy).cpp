...
graph_edge *trace_parser(char *argv[], std::vector<node *> &graph){
	...
	string tmp, syscall, process_comm, parent_proc, filename, file_type, object_type, object_name, subject_type, subject_name, file_path, child_name; //use parent_proc for subject_name,
	...
	while (!logfile.eof()){
		...
		if(line.find(APP) == string::npos || line.find(EXIT_SYSCALL) != string::npos || line.find(SYS_NEWSTAT) !=string::npos )//ignore the "exit_syscall"
			continue;
		
		//parsing the line		
		if(line.find("comm = \"firefox\"") == string::npos && line.find(FILENAME) != string::npos && line.find("sys_execve: ") != string::npos)//find the record has both "comm = <something else>" and "filename =" filed. how the firefox process has been forked
		{ //this branch created the firefox process. 
			subject_type = PROCESS; //the process must be the one that fork firefox.
			object_type = REG;
			file_path = line.substr(line.find(FILENAME)+FILENAME_LEN, line.find_first_of("\"", line.find(FILENAME) + FILENAME_LEN)-(line.find(FILENAME) + FILENAME_LEN));
			subject_name = line.substr(line.find(comm_len)+comm_len.size(),line.find_first_of("\"", line.find(comm_len) + comm_len.size())-(line.find(comm_len) + comm_len.size()));
			object_name = file_path.substr(file_path.rfind("/")+1);
			//child_name = process_comm;
			sub_obj_toggle = 1;
			//debug print 
			cout << "  #!FIREFOX PROCESS CREATED by PROCESS: " << subject_name << endl;
			cout << "  #!FIREFOX PROCESS CREATED (name): " << object_name << endl;
			cout << "  #!FIREFOX PROCESS CREATED (From Path): " << file_path << endl;	
			//cout << "*PROCESS CREATED: " << line.substr(line.find("sys_")) << endl;
			//child_name = process_comm;
		}else if(line.find("comm = \"firefox\"") != string::npos && line.find(FILENAME) != string::npos)//find the record has both "comm = "firefox"" and "filename =" filed
		{
			file_path = line.substr(line.find(FILENAME)+FILENAME_LEN, line.find_first_of("\"", line.find(FILENAME) + FILENAME_LEN)-(line.find(FILENAME) + FILENAME_LEN));
		...
		node *subj = search_graph(graph, subject_name, subject_type);
		node *obj = search_graph(graph, object_name, object_type);
		
		if(!subj){
			subj = new node(subject_type, subject_name);
...
void print_nodes(std::vector<node *> &graph){
	...
	cout << "\nSaman:: The total node in the graph " << graph.size() << "  counter:" << counter << endl;
}
/*
